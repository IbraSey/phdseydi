...
X_BOUNDS = (min(z[:, 0].min() for z in zones), max(z[:, 0].max() for z in zones))
Y_BOUNDS = (min(z[:, 1].min() for z in zones), max(z[:, 1].max() for z in zones))
DURATION = T
...
# Restriction aux événements dans l'union des domaines
model_tmp = SSGCModel.from_polygons(
    polygons=zones,
    duration=DURATION,
    x_bounds=X_BOUNDS,
    y_bounds=Y_BOUNDS,
...
# Filtre spatial : on garde uniquement les points dans le domaine union
domain_union = shapely.unary_union([shapely.Polygon(z) for z in zones])
