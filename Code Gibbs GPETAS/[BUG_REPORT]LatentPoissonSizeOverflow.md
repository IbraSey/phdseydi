...
We address the first solution proposed above.

### Re-define the latent PP to be piecewise homogeneous on the zones of the area source model.

This can be decomposed in the following steps:

1. Define a method of the "SSGC_Gibbs" class, named "simulateZoneHomogeneousPP", which simulates one realization of the time-constant and piecewise homogeneous Poisson process, based on current internal parameters. This amounts to simulate uniform draws separately from each zone, using geopandas as in the following example:
```
import geopandas
...
```
The size of the sample simulated in each zone j is obtained by generating a Poisson variable with mean 
```
self.T * self.eps[j]
```
with self the current instance of the "SSGC_Gibbs" class.

2. Modify method "getRealization" of the "PoissonProcess" class in the "SIGMA_GP_PP_Gibbs" file to comply with the desired result.


3. Modify all other functions and methods of class "PoissonProcess" accordingly. 
